...
describe('Integration tests', () => {
    ...
    test('should cut out glyphs', done => {
        ...
        myFonter.once('data', file => {
            ...
            expect(Object.keys(font.data.cmap).length).toEqual(
                myStringSubset.length
            );
            // Glyphs ascii codes should match
            expect(Object.keys(font.data.cmap)).toEqual(
                myStringSubset.split('').map(l => l.charCodeAt(0).toString())
            );
